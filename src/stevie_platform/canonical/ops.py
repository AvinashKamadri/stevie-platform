...
async def truncate_canonical(conn) -> None:
    ...
    model_predictions (migration 011/014) must survive this: it's keyed by
    (left_key, right_key, model_version) and its candidate_id FK is ON DELETE
    SET NULL specifically so it isn't. But Postgres's TRUNCATE ... CASCADE
    walks the full constraint graph regardless of any FK's own ON DELETE
    action — as long as the constraint exists, truncating `organizations`
    transitively wipes organization_merge_candidate AND, from there,
    model_predictions (verified empirically; ON DELETE SET NULL is simply not
    consulted by TRUNCATE CASCADE). So: null the now-meaningless candidate_id
    pointers, drop that one FK for the duration of the truncate so the cascade
    can't reach model_predictions, then restore it."""
    await conn.execute("update model_predictions set candidate_id = null")
    await conn.execute(
        "alter table model_predictions drop constraint model_predictions_candidate_id_fkey")
    try:
        await conn.execute(
