...
async def _main(argv: list[str]) -> int:
    ...
    ev.add_argument("stage", choices=["subjects", "build", "report", "coverage", "summary", "benchmark"])
    ...
    try:
        if args.cmd == "migrate":
            ...
            if args.stage == "subjects":
                ...
                await evidence.summary_report()
            elif args.stage == "benchmark":
                await evidence.benchmark_report(args.orgs, args.people)
            else:  # build
